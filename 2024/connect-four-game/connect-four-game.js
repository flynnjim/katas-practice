...
ConnectFourGame
Practise your object-oriented skills to create a Connect Four class. The function ConnectFourGame should be called with the new keyword to return a new instance of a Connect Four game. A game should keep track of its state and should have a play method to allow a user to play.

Connect Four is a connection game in which the players first choose a color and then take turns dropping coloured discs from the top into a seven-column, six-row vertically suspended grid. The pieces fall straight down, occupying the next available space within the column. A player wins when they are the first to form a horizontal, vertical, or diagonal line of four of their own discs.
...
The play method should take a column index number and should drop a counter into the correct column. If the column is full, it should return 'This column is full'. The function does not return anything, but the new state of the board can be checked by calling getBoard and the next player can be checked by calling getPlayer.

The checkWinner function should check for a winner in the instance of the Connect Four game. It should return false for no winner, or the winner if there are 4 connected counters in either horizontal, vertical or diagonal directions.
